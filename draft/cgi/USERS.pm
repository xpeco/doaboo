...
sub _init{
	my $self=shift;
	my $user=$self->{db}->DBCONN::rawget('select ADM_PASSWORD,ADM_GROUP,ADM_NAME,ADM_RESTRICTIONS_METHOD,ADM_RESTRICTIONS_FIELD,ADM_RESTRICTIONS_OBJECT,ADM_RESTRICTIONS_SCRIPT,ADM_RESTRICTIONS_MENU, ADM_RESTRICTIONS_REPORT,INITIAL_OBJECT,INITIAL_VIEW,INITIAL_METHOD,INIT_FUNCTION,WIKI_USER,ADM_LANGUAGE,ADM_INST_PER_PAGE,ADM_AUTOMARK_REL,ADM_INIT_EDITOR from ADM_USERS,ADM_GROUPS where ADM_LOGIN=\''.$self->{login}.'\' and ADM_GROUP=ADM_GROUP_NAME and ADM_ENABLE=\'Y\' limit 1');
	$self->{error}=1;
	...
	$self->{adm_password}=$user->[0]->{ADM_PASSWORD};
	$self->{rscripts}=$user->[0]->{ADM_RESTRICTIONS_SCRIPT};
	$self->{rreports}=$user->[0]->{ADM_RESTRICTIONS_REPORT};
	$self->{rmenus}=$user->[0]->{ADM_RESTRICTIONS_MENU};
	$self->{ractions}=$user->[0]->{ADM_RESTRICTIONS_METHOD};
	$self->{rfields}=$user->[0]->{ADM_RESTRICTIONS_FIELD};
	$self->{rtopics}=$user->[0]->{ADM_RESTRICTIONS_OBJECT};
